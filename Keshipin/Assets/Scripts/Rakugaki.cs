        ...
        if(other.tag == "Player")
        {
            if (other.GetComponent<Keshipin_Move>().ReturnMove())
            {
                ...
                if(keshikasuTimer >= keshikasuTime)
                {
                    ...
                    {
                        GameObject keshikasuObj = Instantiate(keshikasu, other.transform.position + -other.GetComponent<Keshipin_Move>().ReturnVector() * (3 * (1 + other.GetComponent<Keshipin_Move>().ReturnKeshikasuNumber()*0.01f)), Quaternion.identity);
